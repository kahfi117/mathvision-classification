...
from utils.smote_fusion import prepare_smote_data_fusion

def train_fusion_pipeline(config_path="../config/fusion_config.yaml"):
    ...
    # 3. MODELS
    print("Initializing Fusion Architecture...")
    ...
    for scenario in SCENARIOS:
        ...
        for lr in LEARNING_RATES:
            ...
            model = FusionBlip2CNN(num_classes, modelBlip2, 
            ...
            if scenario == 'Normal':
                ...
                train_ds.transform = get_transforms()
                resampled_ds = prepare_smote_data_fusion(model, train_ds, BATCH_SIZE, DEVICE)
            ...
            for epoch in range(EPOCHS):
                ...
                for batch in train_bar:
                    ...
                    if 'SMOTE' in scenario:
                        # For SMOTE scenario, batch[0] contains explicitly the (1408+128) fused features
                        feats, labels = batch[0].to(DEVICE), batch[1].to(DEVICE)
                        outputs = model.classifier(feats)
                    else:
                        # For Normal scenarios, pass raw images
                ...
                if val_loss_epoch < best_val_loss:
                    ...
                    torch.save(model.state_dict(), os.path.join(SAVE_DIR, f"fusion_{scenario}_lr{lr}_loss.pth"))
                    print("Best loss model saved!")

                if val_acc_epoch > best_val_acc:
                    best_val_acc = val_acc_epoch
                    torch.save(model.state_dict(), os.path.join(SAVE_DIR, f"fusion_{scenario}_lr{lr}_acc.pth"))
                    print("Best accuracy model saved!")

            final_logs[scenario][f"lr_{lr}"] = history

    # Save final JSON results
    results_path = os.path.join(SAVE_DIR, f"training_results_fusion_{config['run_name']}.json")
